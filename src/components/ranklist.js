...
fetch('https://api.codechef.com/rankings/'+this.props.match.params.contestCode+'?access_token='+this.getAccessToken(), requestOptions)
  ...
  .then(answer=> {
    ...
    for (var i = 0; i < ans.result.data.content.length; i++) {
        participants.push({'rank':ans.result.data.content[i].rank,'name':ans.result.data.content[i].username, 'score':ans.result.data.content[i].totalScore })
    }
