...
class Display extends Component {
  ...
  handleClick = () => {
    window.location.replace(DOMAIN + "/run/");
  };
      ...
      .then(answer =>
        this.setState({
          ...
          Languages: JSON.stringify(
            answer.result.data.content.languagesSupported
          ),
  ...
  render() {
    return (
      <div>
        <h2>{this.state.ProblemName}</h2>
        <p align="right">
          {" "}
          <button onClick={this.clicked}>All Submissions</button>
        </p>
